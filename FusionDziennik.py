                                    ...
                                    if amounttt % 5 == 2:
                                        ...
                                        if choice > len(dostepnePrzedmioty)+len(unikalneNieUczonePrzedmioty):
                                            ...
                                            if choice <= len(dostepnePrzedmioty): 
                                                ...
                                                print('Spróbuj ponownie. \n')                               
                                    if amounttt % 5 == 3:
                                        print('Aby zmienić klasę, usuń tę lekcję, a następnie dodaj nową.')
                                        LessonsScheduleEditor()

                                    if amounttt % 5 == 4:
                                        print('Wybierz dzień (obecnie: {}), na który ma zostać przeniesiony przedmiot {} ({}, nr godziny {}) z klasą {}:'.format(GetComponentOfLessonToEdit['dzien'], GetComponentOfLessonToEdit['przedmiot'], downloadTeacherData(GetComponentOfLessonToEdit['idNauczyciela']), str(GetComponentOfLessonToEdit['nrGodziny']), GetComponentOfLessonToEdit['klasa']))
                                                ...
                                                if wybor == '':
                                                    ...
                                                    if daysOfWeek[wybor] == GetComponentOfLessonToEdit['dzien']:
                                                        ...
                                                        if type(cursor.fetchone()) != NoneType:
                                                            ...
                                                            if type(cursor.fetchone()) != NoneType:
                                                                print('Klasa jest zajęta!')
                                                                daysOfWeek[int(max(daysOfWeek.keys())) + 1]
                                                            else:
                                                                print('Czy na pewno chcesz zmienić dzień lekcji {} ({}) z {} na {}?'.format(GetComponentOfLessonToEdit['przedmiot'], downloadTeacherData(GetComponentOfLessonToEdit['idNauczyciela']), GetComponentOfLessonToEdit['dzien'], daysOfWeek[wybor]))
